<?php
// aplicação de página única de variáveis super globais ($_GET, $_POST, $_SERVER)
...
$buscaProduto = trim((string) ($_GET["produto"] ?? "")); // verificação de nulidade de uma variável (coalescência nula)
...
if($_SERVER["REQUEST_METHOD"] === "POST"){
    //Recuperar os dados de uma formulário
    ...
    if(strlen($nome) < 3){
        $erro["nome"] = "Informe um nome com pelomenos 3 caracteres";
    }

    if (!filter_var($email, FILTER_VALIDATE_EMAIL)){
        $erro["emai"] = "Informe um email válido";
    }

    // Se não existir erros, o cadastro será realizado
    if($erro ===[]){
        $mensagemSucesso = "CAdastro realizado co sucesso!";
    }
}
?>
...
<body>
    ...
    <p>Observar que os dados da pesquisa aparecem na URL</p>
